...
class MentionsController extends Controller
{
  ...
  public $modelClass = 'app\models\Alerts';
  ...
  */
  ...
  public function actionCountMentions($alertId){
    ...
    if($model){
      ...
      $count = Yii::$app->db->createCommand('SELECT COUNT(*) FROM `alerts_mencions` JOIN mentions ON mentions.alert_mentionId = alerts_mencions.id WHERE alertId =:alertId')
           ->bindValue(':alertId', $alertId)
           ->queryScalar();
    ...
    return ['status'=>true,'count'=>$count,'shares' => $shares,'likes' => $likes,'coments' => $coments,'likes_comments' => $likes_comments];
  }
  ...
  public function actionBoxSourcesCount($alertId)
  {
	    ...
	    return ['status' => true,'data' => $data,'modelDataCount' => $modelDataCount];
  }
  ...
  public function actionProductInteration($alertId)
  {
    ...
    return ['status'=>true,'resources'=> $data,'data' => $dataCount];
  }


  /**
   * [actionMentionOnDate interations by date / call component vue: count-date-resources-chart]
   * @param  [type] $alertId [description]
   * @return [type]          [description]
   */
  public function actionMentionOnDate($alertId){
    ...
    return ['status'=>true,'model' => $model,'data' => $data,'resourceNames' => $resourceNames];  
  }
  ...
  public function actionListWords($alertId){
    ...
    return ['status'=>true,'wordsModel' => $wordsModel];
  }
